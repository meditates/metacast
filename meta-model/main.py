...
class Meta_Learning:
    ...
    def eval(self, k, app_num, gradient_steps=10, inner_step_size=0.02,graph=True,testnum=30,myindex=140):
        ...
        index = np.random.choice(myindex,k+testnum,replace=False) #30 is test numbers
...
########this trains the model, you can remove the comment if you want to train the model by yourself
# iterations = 7000
...
#         loss_one = meta.eval(20, index, eval_grad_steps, inner_step_size,graph=False)
#         print("loss is ",loss_one)
#         loss.append(loss_one[1])
# torch.save(meta.model.state_dict(),"maml_7000_319.pth")
##############################

model = Meta_Wave(first_level,second_level)
model.load_state_dict(torch.load('maml_7000_319_2.pth')) 
meta = Meta_Learning(model)

#evaluate speccpu2017 perlbench_r
for index in [360]:
    print(index)
    loss, percent_loss = meta.eval(10, index, eval_grad_steps, inner_step_size,graph=True)

# evaluate speccpu2006 benchmark
armapps = ["lbm","gobmk","bzip2","soplex","mcf"]
for name in armapps:
    temp = pd.read_csv("armapps/"+name+'confs.csv', index_col=0, header=0) # 0-17 columns for features, the 18 is second
...
c=0
percent=[[] for _ in range(len(armapps))]
for i in range(100): #100
    for index in range(362,367): 
        print(index)
        loss, percent_loss = meta.eval(10, index, eval_grad_steps, inner_step_size,graph=False)
        c+= percent_loss
        percent[index-362].append(percent_loss)
print(c/5/100)
plt.figure(figsize=(12, 9))
plt.boxplot(percent,labels=armapps)
plt.title('Boxplot of Percent for benchmarks on ARM')
plt.ylabel('MAPE')
plt.xlabel('SPEC CPU 2006 benchmarks')
plt.savefig('SPEC CPU2006 benchmarks.png')

# evaluate real application
realapps = ["miniqmc","picsarlite","quicksilver","snap","goulash","PENNANT","sw4lite","sw4","minivite","vite","lammps"]
for name in realapps:
    temp = pd.read_csv("realapps1/"+name+'.csv', index_col=0, header=0) # 0-17 columns for features, the 18 is second
    x=temp.values[:,:-1]
    x_norm = scaler.transform(x)
    task_value = np.concatenate((x_norm, temp.values[:, -1:]), axis=1)
    data.append(task_value)

percent=[[] for _ in range(len(realapps))]
for i in range(100):
    for index in range(367,378):
        print(index)
        if index in [367,370,374]:
            loss, percent_loss = meta.eval(10, index, eval_grad_steps, inner_step_size,graph=False,testnum=2,myindex=12)
        elif index in [369,377]:
            loss, percent_loss = meta.eval(10, index, eval_grad_steps, inner_step_size,graph=False,testnum=1,myindex=11)
        else:
            loss, percent_loss = meta.eval(10, index, eval_grad_steps, inner_step_size,graph=False,testnum=3,myindex=13)
        percent[index-367].append(percent_loss)
plt.figure(figsize=(12, 9))
plt.boxplot(percent, labels=realapps)
#plt.xlabel('Arrays')
plt.ylabel('MAPE')
plt.xticks(rotation=35, ha="right")
plt.title('Boxplot for realapps')
plt.savefig('real applications.png')
