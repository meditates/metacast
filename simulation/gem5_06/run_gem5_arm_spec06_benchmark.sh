...
RUN_DIR=$SPEC_DIR/benchspec/CPU2006/$BENCHMARK_CODE/run/run_base_ref\_arm.0000     # Run directory for the selected SPEC benchmark
...
$GEM5_DIR/build/ARM/gem5.fast --outdir=$OUTPUT_DIR $GEM5_DIR/configs/example/spec06_config.py --benchmark=$BENCHMARK --benchmark_stdout=$OUTPUT_DIR/$BENCHMARK.out --simpoint-profile --simpoint-interval=100000000 --cpu-type=NonCachingSimpleCPU  --benchmark_stderr=$OUTPUT_DIR/$BENCHMARK.err | tee -a $SCRIPT_OUT
